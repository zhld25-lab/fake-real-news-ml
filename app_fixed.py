...
def show_model_status(model: Any | None, model_error: str | None) -> None:
    ...
    if model_error == "missing":
        st.warning(
            ...
            "next to app.py before running predictions. Other pages can still be displayed."
        )
...
def render_performance() -> None:
    ...
    st.dataframe(performance_table, hide_index=True, use_container_width=True)

    st.subheader("ROC Curve")
    if ROC_CURVE_PATH.exists():
        st.image(str(ROC_CURVE_PATH), caption="ROC Curve - Final Model", use_container_width=True)
    else:
...
def render_confusion_matrix() -> None:
    ...
    st.subheader("Matrix")
    st.dataframe(format_confusion_matrix(), use_container_width=True)
...
def render_classification_report() -> None:
    ...
    st.dataframe(format_classification_report(), hide_index=True, use_container_width=True)

    with st.expander("Report notes"):
        st.write("Label convention used by the app: 0 = Fake News, 1 = True News.")
        st.write("Accuracy: 0.9994 on 8,935 held-out examples.")
...
def render_bootstrap_intervals() -> None:
    ...
    st.dataframe(format_bootstrap_intervals(), hide_index=True, use_container_width=True)
